...
ISR:         ADD  R1,0x01              ;the botton 7 LEDs will should increment
             CMP  R1,0x90              ;Note that 0x80 + 0x10 = 0x90, which means interrupts 
                                       ;are enabled and the 16th interrupt was counted
                                       ;so it is time to quite interrupts.
                                     
             BREQ DISABLE_INTS
	         OUT  R1,led_port          ;everytime an interrupt is received.
	         RETIE

DISABLE_INTS:
             OR   R1,0x40              ;illuminate LD4 indicating interrupts disabled
             OUT  R1,led_port
             MOV  R2,0xFF              ;set flag to disallow interrupts from being 
                                       ;enabled again in the main program
             RETID
